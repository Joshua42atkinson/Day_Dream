...
async fn fetch_profile_data() -> Option<ProfileData> {
    let req_url = "[http://127.0.0.1:3001/api/profile_data](http://127.0.0.1:3001/api/profile_data)";
...
async fn fetch_player_character() -> Option<PlayerCharacter> {
    let req_url = "[http://127.0.0.1:3001/api/player_character](http://127.0.0.1:3001/api/player_character)";
    reqwest::get(req_url).await.ok()?.json::<PlayerCharacter>().await.ok()
}

async fn fetch_journal_data() -> Option<JournalData> {
    let req_url = "[http://127.0.0.1:3001/api/journal_data](http://127.0.0.1:3001/api/journal_data)";
...
async fn submit_command_to_api(command: PlayerCommand) -> Result<GameTurn, ServerFnError> {
    let client = reqwest::Client::new();
    let res = client
        .post("[http://127.0.0.1:3001/api/submit_command](http://127.0.0.1:3001/api/submit_command)")
        .json(&command)
        .send()
        .await
    ...
    let submit_action = create_action(
        move |input: &String| {
            ...
            if char_opt.is_none() {
                ...
                return async { Err(ServerFnError::ServerError("Character not loaded".to_string())) };
            }
            ...
            async move { submit_command_to_api(command).await }
        }
    );
        ...
        <div class="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="md:col-span-2">
                <section class="leptos-panel border border-gray-700 rounded-lg p-6">
                    ...
                    // (IMPROVEMENT) This is now a <Form> that dispatches the action
                    <ActionForm action=submit_action>
                        <input
                            type="text"
                            name="command" // The action needs a name
